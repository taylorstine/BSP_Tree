...
bool Mesh::load(char * filename){
    ...
    for ( size_t i = 0; i < face_list.size(); ++i ) {
	    ...
	    this->indices.push_back(rv.first->second);
        }
	this->normals.push_back(get_normal(vertices[tri.indices[0]].position,
					   vertices[tri.indices[1]].position,
					   vertices[tri.indices[2]].position));
	tri.normal_index = normals.size()-1;
        this->triangles.push_back( tri );
	this->num_triangles++;
    }
    this->COM/=num_vertices;
...
Vector3 Mesh::get_normal(Vector3 v0, Vector3 v1, Vector3 v2){
  Vector3 a = v1-v0;
  Vector3 b = v2-v0;
...
void Mesh::calculate_normals(){
  ...
  for(i =0; i < triangles.size(); i++){
    ...
    b = vertices[triangles[i].indices[0]].position-
