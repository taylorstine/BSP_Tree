...
void Bnode::add(Triangle tri){
  ...
  a = V0( tri ); a_idx = tri.indices[ 0 ];
  b = V1( tri ); b_idx = tri.indices[ 1 ];
  c = V2( tri ); c_idx = tri.indices[ 2 ];
    
  //find the (signed) distance of each vertex to the
  //current partitioning plane
  fa = distance_to_plane( P0( tri ), current_normal, current_q0 );
  fb = distance_to_plane( P1( tri ), current_normal, current_q0 );
  fc = distance_to_plane( P2( tri ), current_normal, current_q0 );
